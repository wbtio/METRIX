...
export default function Home() {
  ...
  const fetchTaskStats = useCallback(
    async (goalIds: string[]) => {
      ...
      for (const task of tasks || []) {
        if (!statsMap[task.goal_id])
          statsMap[task.goal_id] = { completed: 0, total: 0 };
  ...
  const executeNavigation = useCallback(
    (navigation: PendingNavigation, options?: { discardAiDraft?: boolean }) => {
      ...
      if (
        ...
        navigation.selectedGoalId !== undefined
      ) {
      ...
      >
        {loading ? (
          <div className="flex flex-col items-center justify-center animate-pulse gap-4">
            ...
            <p className="text-primary/50 font-medium tracking-widest text-sm">
              {t.loadingOrbit}
            </p>
          </div>
        ) : currentView === "home" ? (
