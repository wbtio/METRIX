...
export function HistoryCard({ history, historyLoading, locale, ui, t, embedded = false }: HistoryCardProps) {
  ...
  const innerPanelClass =
    'mt-3 rounded-xl border border-border/40 bg-background p-3 sm:p-4';
      ...
      <section className={cn(!embedded && cardClass, !embedded && shellClass)}>
        ...
        >
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[10px] bg-muted/40 text-muted-foreground border border-border/40">
              <History className="h-4 w-4" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="text-sm font-bold tracking-tight text-foreground">{ui.historyTitle}</div>
              <div className="text-xs text-muted-foreground">{emptyMeta}</div>
            </div>
          </div>
  ...
  if (history.length === 0) {
    return (
      <section className={cn(!embedded && cardClass, !embedded && shellClass)}>
        ...
        {!collapsed && (
          <div className={innerPanelClass}>
            <div className="rounded-xl border border-dashed border-border/40 bg-muted/30 px-3 py-6 text-center text-sm text-muted-foreground">
              {ui.archiveEmpty}
            </div>
          </div>
        )}
      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-muted-foreground">{historyMeta}</div>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <span className="inline-flex h-fit items-center rounded-full border border-border/40 bg-muted/30 px-2.5 py-1 text-[10px] font-semibold text-muted-foreground">
            {history.length}
          </span>
          ...
          <div className="space-y-2 max-h-[22rem] overflow-y-auto pr-0.5 scrollbar-thin sm:max-h-[24rem] lg:max-h-[26rem]">
            {history.map((item) => (
              <div
                ...
                className={cn(
                  softCardClass,
                  'rounded-xl border-border/40 bg-background px-3 py-2.5',
                )}
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 text-xs leading-5 text-muted-foreground sm:hidden">
                  ...
                  <div className="flex items-center justify-between gap-3 sm:ms-auto sm:min-w-[11rem] sm:justify-end">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-xs font-semibold text-muted-foreground">{t.challengeOpponent}</div>
                      <PlayerAvatar
