...
export default function DashboardHeader({
  ...
  const goalEndDaysChip = getGoalEndDaysChip(goal.estimated_completion_date, isArabic);
    ...
    <div className="rounded-2xl border border-border/80 bg-white dark:bg-card/50 p-3 sm:p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0" dir={isArabic ? 'rtl' : 'ltr'}>
          <GoalIconPicker currentIconName={goal.icon || 'Target'} onSelect={onUpdateIcon}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              ...
              {goalEndDaysChip && (
                <span
                  className={cn(
                    'flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full tabular-nums',
                    goalEndDaysChip.tone === 'soon' && 'bg-primary/10 text-primary dark:text-primary',
                    goalEndDaysChip.tone === 'today' && 'bg-amber-500/10 text-amber-700 dark:text-amber-400',
                    goalEndDaysChip.tone === 'late' && 'bg-destructive/10 text-destructive dark:text-destructive'
                  )}
                  title={goalEndDaysChip.title}
                >
                  <Clock className="w-3 h-3 shrink-0" aria-hidden />
                  {goalEndDaysChip.text}
                </span>
              )}
              {streak > 0 && (
        ...
        <div className="flex items-center gap-1 shrink-0">
          <DropdownMenu>
            ...
            <DropdownMenuContent align={isArabic ? 'start' : 'end'} className="w-52">
              ...
              <DropdownMenuItem onClick={onToggleDetails} className="cursor-pointer">
                <Info className="w-4 h-4" />
                <span>{showGoalDetails ? (isArabic ? 'إخفاء التفاصيل' : 'Hide Details') : (isArabic ? 'عرض التفاصيل' : 'Show Details')}</span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={onDeleteGoal} variant="destructive" className="cursor-pointer">
        ...
        <div className="pt-2 border-t border-border/60 animate-in fade-in slide-in-from-top-2 duration-200" dir={isArabic ? 'rtl' : 'ltr'}>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            <div className="bg-muted/30 rounded-xl p-2.5">
              <p className="text-[10px] text-muted-foreground font-semibold mb-0.5">{isArabic ? 'تاريخ البدء' : 'Start Date'}</p>
              <p className="text-xs font-bold text-foreground">
                {new Date(goal.created_at).toLocaleDateString(isArabic ? 'ar-SA' : 'en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
              </p>
            </div>
            <div className="bg-muted/30 rounded-xl p-2.5">
              <p className="text-[10px] text-muted-foreground font-semibold mb-0.5">{isArabic ? 'تاريخ الانتهاء' : 'End Date'}</p>
              <p className="text-xs font-bold text-foreground">
            ...
            <div className="bg-muted/30 rounded-xl p-2.5">
              <p className="text-[10px] text-muted-foreground font-semibold mb-0.5">{isArabic ? 'النقاط الحالية' : 'Current Points'}</p>
              <p className="text-xs font-bold text-foreground">{goal.current_points.toLocaleString()}</p>
            </div>
            <div className="bg-muted/30 rounded-xl p-2.5">
              <p className="text-[10px] text-muted-foreground font-semibold mb-0.5">{isArabic ? 'النقاط المستهدفة' : 'Target Points'}</p>
              <p className="text-xs font-bold text-foreground">{goal.target_points.toLocaleString()}</p>
            </div>
            <div className="bg-muted/30 rounded-xl p-2.5">
          ...
          {goal.ai_summary && (
            <div className="mt-2 bg-primary/5 rounded-xl p-2.5 border border-primary/10">
              <p className="text-[10px] text-primary/70 font-semibold mb-1">{t.goalDescription}</p>
