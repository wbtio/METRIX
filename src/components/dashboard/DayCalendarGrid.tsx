...
export default function DayCalendarGrid({
  ...
  const goalStart = useMemo(() => parseLocalDay(goalStartDate), [goalStartDate]);
  ...
  const { calendarCells, monthStats } = useMemo(() => {
    ...
    const cells: Array<CalendarCell | null> = Array.from({ length: 42 }, (_, index) => {
    ...
    const validMonthCells = cells.filter((cell): cell is CalendarCell => cell !== null);
    ...
    return {
      ...
      monthStats: {
        ...
        totalPoints: loggedCells.reduce((sum, cell) => sum + cell.totalPoints, 0),
    ...
    () =>
      Array.from({ length: 12 }, (_, monthIndex) => {
        ...
        return {
          ...
          isActive: viewDate.getFullYear() === pickerYear && viewDate.getMonth() === monthIndex,
        };
      }),
  ...
  return (
    <>
      <div
        className="relative flex h-full flex-col overflow-hidden rounded-xl border border-border/70 bg-card/35 p-1.5 shadow-xl ring-1 ring-border/10 backdrop-blur-xl sm:p-2"
      ...
      >
        ...
        <div className="border-b border-border/60 pb-0.5">
          <div className="flex items-center">
            <Popover open={isMonthPickerOpen} onOpenChange={handleMonthPickerOpenChange}>
              <PopoverTrigger asChild>
                <button
                  type="button"
                  className="group flex min-w-0 flex-1 items-center justify-center gap-1 rounded-lg px-0.5 py-0.5 text-center transition-colors hover:bg-muted/35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:ring-offset-2 focus-visible:ring-offset-card"
            ...
            {calendarCells.map((day, index) => {
              ...
              return (
                <button
                  key={day.date}
                  type="button"
                  onClick={() => isSelectable && setSelectedDate(isSelected ? null : day.date)}
                  onMouseEnter={(event) => {
                ...
                >
                  <span
                    className={cn(
                      'absolute left-1.5 top-1.5 text-[10px] font-semibold',
                      day.isBeforeStart || day.isAfterToday
        ...
        <div className="mt-2 flex items-center justify-between gap-2 border-t border-border/60 pt-1">
          <div className="min-w-0 flex flex-1 items-center overflow-hidden">
            <div className="inline-flex max-w-full items-center gap-1.5 overflow-hidden rounded-full border border-border/70 bg-muted/15 px-2 py-1 text-[9px] font-medium leading-none whitespace-nowrap sm:px-2.5 sm:text-[10px]">
              {monthSummaryItems.map((item, index) => (
                <span key={item.key} className="inline-flex items-center gap-1.5">
                  <span className="inline-flex items-center gap-1 whitespace-nowrap">
                    <span className={cn('font-semibold tabular-nums', item.accent)}>{item.value}</span>
                    <span className="text-muted-foreground">{item.label}</span>
                  </span>
                  {index < monthSummaryItems.length - 1 && (
                    <span aria-hidden="true" className="h-3 w-px shrink-0 rounded-full bg-border/60" />
                  )}
                </span>
              ))}
            </div>
          ...
          <div className="inline-flex shrink-0 items-center justify-end gap-1 text-[9px] text-muted-foreground whitespace-nowrap sm:gap-1.5 sm:text-[10px]">
            <span>{labels.less}</span>
            {[0, 1, 2, 3, 4].map((intensity) => (
              <div
                key={intensity}
                className={cn('h-2.5 w-2.5 rounded-[4px] border sm:h-3 sm:w-3', heatClass(intensity))}
              />
            ))}
      ...
      {selectedDate && (
        <div className="fixed inset-0 z-[90] flex items-end justify-center" onClick={() => setSelectedDate(null)}>
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3 border-b border-border px-4 pb-3">
              <div>
                <p className="text-[10px] text-muted-foreground">{labels.dayDetail}</p>
                ...
                {selectedLogs.length > 0 && (
                  <div className="flex items-center gap-1 rounded-xl bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary ring-1 ring-primary/20">
                    ...
                    <span>
                      +
                      {formatShortNumber(
                        selectedLogs.reduce((sum, log) => sum + (log.ai_score || 0), 0),
                        isArabic,
                      )}
                    </span>
                  </div>
